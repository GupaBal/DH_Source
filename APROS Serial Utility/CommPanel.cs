...
namespace APROS_Serial_Utility
{
    ...
    public partial class CommPanel : UserControl
    {
        ...
        private void m_btn_Click(object sender, EventArgs e)
        {
            ...
            else if (sender == m_btn_send)
            {
                ...
                else
                {
                    ...
                    {
                        AppendTextBox(m_txb_recv, m_txb_send.Text);
                    }
                }
            }
        ...
        ///=========================================================================================================///
        /// <summary>                                                                     [ Ver 1.00 / 2017-05-31 ] ///
        /// @ m_cb_sendHistory_SelectedIndexChanged @                                                               ///
        ///     이전 입력 데이터 중 선택된 항목을 전송 텍스트박스에 입력한다.                                       ///
        /// </summary>                                                                                              ///
        /// <param name="sender"> object : 이벤트 발생 객체 </param>                                                ///
        /// <param name="e"> EventArgs : 이벤트 관련 정보 </param>                                                  ///
        ///=========================================================================================================///
        private void m_cb_sendHistory_SelectedIndexChanged(object sender, EventArgs e)
        {
            m_txb_send.Text = m_cb_sendHistory.Text;
        }
        ...
        private void serialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            FileManager.LogWriter(CommonBase.Hex2Str16(buff, buff.Length), FPath, FName, false, false, false, false, true);
        }
        ...
        private void timer_getter_Tick(object sender, EventArgs e)
        {
            if (Server_Mode == true)
            {
                foreach(TCP_Comm_Manager tcm in Server.GetClients())
                {
                    ...
                    {
                        TmpString = tcm.GetRemoteIP() + "(" + tcm.GetRemotePort() + ") : ";

                        AppendTextBox(m_txb_recv, TmpString);
                        AppendText(packet);
            ...
            else
            {
                ...
                foreach (byte[] packet in Packets)
                {
                    TmpString = Client.GetRemoteIP() + "(" + Client.GetRemotePort() + ") : ";
        ...
        private bool Send()
        {
            try
            {
                if (m_rbtn_sendH.Checked == true)
                {
                    TmpString = m_txb_send.Text.Replace(Environment.NewLine, " ");
                    Send_Buffer = CommonBase.HexStringToByteArray(TmpString);
                }
                else
                {
                    TmpString = m_txb_send.Text;
                ...
                m_cb_sendHistory.Items.Add(TmpString);
        ...
        ///=========================================================================================================///
        private void AppendText(byte[] buff)
        {
            ...
            {
                AppendTextBox(m_txb_recv, Encoding.UTF8.GetString(buff));
            }
            else
            {
                AppendTextBox(m_txb_recv, CommonBase.Hex2Str16(buff, buff.Length));
            }
        ...
        public string SaveConfig()
        {
            ...
            int ICnt = m_cb_sendHistory.Items.Count;
            ...
            if (ICnt > 0)
            {
                ...
                {
                    history += m_cb_sendHistory.Items[i].ToString();
            ...
            for (int i = 0; i < values.Length; i++)
            {
                ...
                switch(sub[0])
                {
                    ...
                    case "History":
                        ...
                        {
                            m_cb_sendHistory.Items.Add(sub[i]);
                        }
                        break;
                    default: break;
                }
